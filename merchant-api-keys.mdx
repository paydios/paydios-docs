---
title: "PkPay+ API Keys: Create, Rotate, and Secure"
sidebarTitle: "API Keys"
description: "Manage your PkPay+ secret and publishable API keys. Learn how to create keys, use them in requests, rotate them, and keep them secure."
icon: "key"
---

PkPay+ uses API keys to authenticate every request you make to the Merchant API. You have two types of key — a secret key for server-side calls and a publishable key for client-side checkout flows. This page explains the difference, how to create and manage your keys, and how to keep them secure.
...
Log in to the [PkPay+ dashboard](https://merchant.pkpayplus.com/dashboard/api-keys) and navigate to **Dashboard → API Keys**. Both keys are displayed on this page alongside your Merchant ID.
...
Store your secret key in an environment variable rather than hard-coding it in source files. The PkPay+ PHP SDK reads this variable automatically.
...
Once `PKPAYPLUS_SECRET_KEY` is set in your environment, create the client without any arguments:
...
<Steps>
  <Step title="Open the API Keys page">
    Log in to [merchant.pkpayplus.com/dashboard/api-keys](https://merchant.pkpayplus.com/dashboard/api-keys).
  </Step>
...
- **Use environment variables** — store `PKPAYPLUS_SECRET_KEY` in `.env` locally and your hosting provider's secret manager in production. Never hard-code key values in source files.
